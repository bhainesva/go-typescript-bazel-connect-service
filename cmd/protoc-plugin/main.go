...
func generate(plugin *protogen.Plugin, file *protogen.File) {
	...
	generateServiceNameVariables(generatedFile, file, grpcPackage)
	for _, service := range file.Services {
		generateService(generatedFile, service, grpcPackage)
	}
}
...
func generateServiceNameVariables(g *protogen.GeneratedFile, file *protogen.File, pt protogen.GoImportPath) {
	file.GoImportPath = pt
	file.GoDescriptorIdent.GoImportPath = pt
...
func generateService(g *protogen.GeneratedFile, service *protogen.Service, path protogen.GoImportPath) {
	names := newNames(service)
	generateClientInterface(g, service, names, path)
	generateClientImplementation(g, service, names, path)
	generateServerInterface(g, service, names, path)
	generateServerConstructor(g, service, names)
	generateUnimplementedServerImplementation(g, service, names, path)
}

func generateClientInterface(g *protogen.GeneratedFile, service *protogen.Service, names names, pt protogen.GoImportPath) {
	...
	for _, method := range service.Methods {
		...
		g.P(clientSignature(g, method, false /* named */, pt))
	}
	g.P("}")
	g.P()
}

func generateClientImplementation(g *protogen.GeneratedFile, service *protogen.Service, names names, pt protogen.GoImportPath) {
	...
	g.P()
	for _, method := range service.Methods {
		generateClientMethod(g, method, names, pt)
	}
}

func generateClientMethod(g *protogen.GeneratedFile, method *protogen.Method, names names, pt protogen.GoImportPath) {
	...
	g.P("func (c *", receiver, ") ", clientSignature(g, method, true /* named */, pt), " {")
...
func clientSignature(g *protogen.GeneratedFile, method *protogen.Method, named bool, pt protogen.GoImportPath) string {
	reqName := "req"
	ctxName := "ctx"
	if !named {
		reqName, ctxName = "", ""
	...
	return method.GoName + serverSignatureParams(g, method, named, pt)
}

func generateServerInterface(g *protogen.GeneratedFile, service *protogen.Service, names names, path protogen.GoImportPath) {
	...
	for _, method := range service.Methods {
		...
		g.P(serverSignature(g, method, path))
	}
	g.P("}")
	g.P()
}
...
func generateUnimplementedServerImplementation(g *protogen.GeneratedFile, service *protogen.Service, names names, pt protogen.GoImportPath) {
	...
	g.P()
	for _, method := range service.Methods {
		g.P("func (", names.UnimplementedServer, ") ", serverSignature(g, method, pt), "{")
		if method.Desc.IsStreamingServer() {
...
func serverSignature(g *protogen.GeneratedFile, method *protogen.Method, path protogen.GoImportPath) string {
	return method.GoName + serverSignatureParams(g, method, false, path /* named */)
}

func serverSignatureParams(g *protogen.GeneratedFile, method *protogen.Method, named bool, pt protogen.GoImportPath) string {
	ctxName := "ctx "
	reqName := "req "
	streamName := "stream "
	if !named {
	...
	method.Input.GoIdent.GoImportPath = pt
	method.Output.GoIdent.GoImportPath = pt
	if method.Desc.IsStreamingClient() && method.Desc.IsStreamingServer() {
		// bidi streaming
